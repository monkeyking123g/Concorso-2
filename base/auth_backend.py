...
class AuthBackend(authentication.BaseAuthentication):
    ...
    def authenticate(self, request, token = None, **kwargs) -> Optional[tuple]:
        ...
        if len(auth_header) == 1:
            raise exceptions.AuthenticationFailed('Invalid token header. No credentionl provider')
        elif len(auth_header) > 2:
            raise exceptions.AuthenticationFailed('Invalid token header. Token string shoould not contain spaces')
        ...
        try:
            ...
            raise exceptions.AuthenticationFailed(
                'Invalid token header. Token string should not cantain invalid characters.'
            )
    ...
    def authenticate_credential(self, token) -> tuple:
        ...
        try:
            ...
            raise exceptions.AuthenticationFailed('No user matching thes token was found.')
